...
const LoanList = () => {
    ...
    const getLoan = async () => {
        // const response = await axios.get("http://172.25.164.15:3667/loan");
        const response = await axios.get("http://localhost:3667/loan");
        ...
        <CRow>
            <CButtonGroup role="group" aria-label="Basic example" style={{ marginBottom: "5px", borderRadius: '25px' }}>
                <CButton color="dark" style={{ border: "solid 2px orange", margin: '2px', borderRadius: '15px' }} onClick={() => setVisible(!visible)}>
                    <HiViewGridAdd style={{ color: "orange" }} />  New Request
                </CButton>
                ...
                <CButton color="dark" style={{ border: "solid 2px orange", margin: '2px', borderRadius: '15px' }}>
                    <Link to="/base/rejectedLoan" style={{ color: "white", textDecoration: "none", fontWeight: 700 }}>
                        <FiLoader style={{ color: "orange" }} />  Rejected Request
                    </Link>
                </CButton>
                        ...
                        {loan.map((loan, index) => (
                            <tr key={loan.uid}>
                                ...
                                <td>
                                    ...
                                    {user && user.role === "ceo" && (
                                        <CButton color="primary" style={{ marginLeft: "4px" }} id="editUsers">
                                            <Link id="editLoan" style={{ textDecoration: "none", fontWeight: 700, color: 'white' }}
                                                to={`/base/LoanList/edit/${loan.uid}`}
